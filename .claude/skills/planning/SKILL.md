...
3. **Open Source Reference Mode** → Search, clone, and analyze similar open source projects
...
## Open Source Reference Mode (`/plan:ref`)

When the user wants to reference similar open source projects before implementing.

### Activation
User says one of:
- "tham khảo open source"
- "tìm open source tương tự"
- "reference open source"
- Use command: `/plan:ref <task>`

### Steps
1. **Search & Identify**: Use `researcher` agent to find best-in-class open source solutions in the domain
2. **Clone & Analyze**: Pull top repositories to `./reference/` for detailed analysis
3. **Document Insights**: Create `reference-analysis.md` documenting:
   - Key architectural patterns
   - Code structures and patterns
   - Design decisions and trade-offs
   - Lessons learned
4. **Apply to Plan**: Integrate insights into implementation plan phases
...
└── {project-name}/
    ...
    ├── reference-analysis.md  # Your analysis
    └── (cloned code)          # Cloned source code
```

### Rules
- **Do NOT commit** reference folder to git (add to .gitignore)
- **Focus on quality**: Choose well-maintained, popular projects
- **Document learnings**: Be specific about what patterns to adopt/avoid
- **Stay scoped**: Only analyze relevant parts, don't get lost in exploration
